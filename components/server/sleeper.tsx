...
const Sleeper = ({ n, Price,handleNoOfPassangers }: SleeperProps) => {
  ...
  return (
    <div className="w-14 relative flex justify-start">
      <div className={green[n] ? " bg-green-600 h-16 w-11 rounded-lg flex justify-center items-center mt-3 cursor-pointer transition-transform ease-in-out duration-200 transform border-0 hover:scale-120" : " h-16 w-11 bg-slate-100 rounded-lg flex justify-center items-center mt-3 cursor-pointer transition-transform ease-in-out duration-200 transform border-0 hover:scale-110"}
        onClick={() => { handleClick(n); handleNoOfPassangers(n, 1) }}>
        {n}
      </div>
      <div className={green[n] ? "absolute left-full top-[40%] bg-slate-800 text-white font-semibold rounded p-1" : "hidden absolute right-full top-[40%] bg-slate-800 text-white rounded p-1"}>
        ₹{Price[n]}
      </div>
    </div>
  )
